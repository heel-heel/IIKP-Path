...
def compute_medians(results, task_name):
    ...
    total_median_mar = np.median(all_mar_diffs) if all_mar_diffs else 0
    total_median_mnar = np.median(all_mnar_diffs) if all_mnar_diffs else 0
...
def save_results_to_file(all_results, output_file="analysis_results.txt"):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for task_name, results in all_results.items():
            ...
            medians = compute_medians(results, task_name)
            ...
            # 写入数据集中位数结果
            f.write(f"\n--- 数据集中位数结果 ---\n")
            f.write(f"{'数据集':<20} {'MAR中位数差值':<20} {'MNAR中位数差值':<20}\n")
            f.write("-" * 60 + "\n")
            ...
            # 写入填补算法中位数结果
            f.write(f"\n--- 填补算法中位数结果 ---\n")
            f.write(f"{'算法':<15} {'MAR中位数差值':<20} {'MNAR中位数差值':<20}\n")
            f.write("-" * 55 + "\n")
            ...
            # 写入总中位数
            f.write(f"\n--- 总中位数结果 ---\n")
            f.write(f"MAR总中位数差值: {medians['total_median_mar']:.6f}\n")
            f.write(f"MNAR总中位数差值: {medians['total_median_mnar']:.6f}\n")

            # ========== 详细结果 ==========
            f.write(f"\n{'=' * 40}\n")
            f.write("【详细结果（每个数据集、每个算法）】\n")
            f.write(f"{'=' * 40}\n")
            for dataset in results:
                f.write(f"\n数据集: {dataset}\n")
...
def main():
    ...
    # 保存结果到文件
    output_file = os.path.join("results", "missing_mechanism_analysis_results3.txt")
    save_results_to_file(all_results, output_file)
    print("\n分析完成！结果已保存到 'missing_mechanism_analysis_results3.txt'")
