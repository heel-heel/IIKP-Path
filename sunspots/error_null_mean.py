...
# 计算Ozone列的平均值（除去NaN）
average_ozone = data['Sunspots'].mean()

# 定义一个函数来填充NaN值
def fill_nan_with_neighbors(index, data, average_ozone):
    row = data.iloc[index]
    if pd.isna(row['Sunspots']):
        # 寻找前面的最近的非空值
        previous = np.nan
        for i in range(index-1, -1, -1):
            if not pd.isna(data['Sunspots'][i]):
                previous = data['Sunspots'][i]
                break
        # 寻找后面的最近的非空值
        next = np.nan
        for i in range(index+1, len(data)):
            if not pd.isna(data['Sunspots'][i]):
                next = data['Sunspots'][i]
                break
...
# 应用函数填充NaN值
data['Sunspots'] = data.index.map(lambda x: fill_nan_with_neighbors(x, data, average_ozone))
