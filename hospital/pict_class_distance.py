...
csv_files = [f'dirty-City-{model}-{p}.csv' for model in model_names for p in percentages]
...
for file in csv_files:
    ...
    if file == "clean.csv":
        ...
        model = file.split('-')[2]
        read_path = f'null-City-{model}'
        data = pd.read_csv(os.path.join(read_path,file))

    # 将 "empty" 转换为 NaN
    data['Score'] = data['Score'].replace('empty', np.nan)
    ...
    def convert_to_decimal(x):
        try:
            ...
            # 如果无法转换（例如已经是 NaN），直接返回原值
            return x


    # 应用转换函数
    data['Score'] = data['Score'].apply(convert_to_decimal)

    # 用均值填充 NaN 值
    data['Score'] = data['Score'].fillna(data['Score'].mean())

    data.columns = data.columns.str.lower()
    features = data.drop(columns=['city'])
    target = data['city']
