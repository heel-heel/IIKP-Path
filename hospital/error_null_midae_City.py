...
def process_and_fill_csv(file_name, output_file_name):
    ...
    # 删除除city列以外其他列有缺失值的行
    df_train = df.dropna(subset=[col for col in df.columns if col != 'City' and col != 'ProviderNumber'])
    ...
    # 归一化数据
    scaler = MinMaxScaler()
    X_scaled = scaler.fit_transform(df_train.drop(columns=['ProviderNumber']))
