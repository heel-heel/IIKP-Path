...
def process_and_fill_csv(file_name, output_file_name):
    ...
    # 删除缺失值行以训练GAN
    df_train = df.drop(columns=['ProviderNumber']).fillna(0)
    ...
    for index in missing_indices:
        ...
        # 确保生成的数值在已知的城市编码范围内
        known_city_codes = df_train['City'].values
        ...
        df.at[index, 'City'] = city_name
        df_copy.at[index, 'City'] = df.at[index, 'City']
    df_copy.to_csv(os.path.join(save_path,output_file_name), index=False)

# 主程序
file_numbers = [10,30,50,70,90]
for number in file_numbers:
    input_file = f'dirty-City-{number}.csv'
    output_file = f'dirty-City-gan-{number}.csv'
