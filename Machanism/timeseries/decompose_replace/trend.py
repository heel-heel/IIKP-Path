...
for dataset, columns in datasets.items():
    ...
    base_path = "../../../Datasets"
    output_path = os.path.join(base_path, dataset, "Machanism", "timeseries", "decompose_replace", "trend")
    if not os.path.exists(output_path):
        os.makedirs(output_path)

    for rate, model_trend in current_missing_rate_to_model.items():
        for model_original in models_original:
            input_trend_file = os.path.join(base_path, dataset, "Imputation", f"null-{model_trend}", f"dirty-{model_trend}-{rate}.csv")
