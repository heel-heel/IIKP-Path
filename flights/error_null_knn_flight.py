...
def fill_city_column(file_path):
    ...
    # 获取没有缺失值的记录
    non_missing = df[df['flight'].notnull()]
    # 获取缺失值的记录
    ...
    for index, row in missing.iterrows():
        ...
        # 计算这5个city值的众数
        mode_flight = pd.Series(nearest_cities).mode()[0]

        # 将众数赋值给df中index对应的记录的city列
        df.loc[index, 'flight'] = mode_flight
...
# 文件列表
file_names = ['dirty-flight-10.csv', 'dirty-flight-30.csv', 'dirty-flight-50.csv', 'dirty-flight-70.csv', 'dirty-flight-90.csv']

# 处理每个文件并保存结果
for file_name in file_names:
    filled_df = fill_city_column(file_name)
    filled_file_name = file_name.replace('dirty-flight', 'dirty-flight-knn')
