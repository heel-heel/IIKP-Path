...
for rate in missing_rates:
    ...
    output_filename = f'dirty-flight-mode-{rate}.csv'
    ...
    # 步骤3: 找到“city”列的众数
    # 确保“city”列不为空再找众数
    if not df['flight'].empty:
        city_mode = df['flight'].mode()[0]  # mode()返回的是一个Series，取第一个值作为众数

        # 步骤4: 用众数填充“city”列中的缺失值
        df['flight'].fillna(city_mode, inplace=True)
