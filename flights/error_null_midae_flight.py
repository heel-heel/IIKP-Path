...
def process_and_fill_csv(file_name, output_file_name):
    ...
    # 删除除city列以外其他列有缺失值的行
    df_train = df.dropna(subset=[col for col in df.columns if col != 'flight'])

    # 均值填充city列缺失值，以便训练DAE模型
    df_train['flight'] = df_train['flight'].fillna(df_train['flight'].mean())
    ...
    # 获取已知的城市编码范围
    known_city_codes = df_train['flight'].dropna().values
    min_code = np.min(known_city_codes)
    max_code = np.max(known_city_codes)

    # 填充city列的缺失值，但不改变非空缺位置的数值
    city_index = df.columns.get_loc('flight')
    missing_indices = df_copy[df_copy['flight'].isnull()].index
...
for rate in Missing_rate:
    ...
    output_file = f'dirty-flight-midae-{rate}.csv'
