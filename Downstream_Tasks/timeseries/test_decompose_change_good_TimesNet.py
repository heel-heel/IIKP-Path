...
def create_timesnet_dataset(data, seq_len, label_len, pred_len, dataset):
    """创建TimesNet数据集"""
    x_enc_list = []
    x_mark_enc_list = []
    x_dec_list = []
    ...
    for i in range(total_len - total_seq_len + 1):
        ...
        # 时间特征占位符
        if dataset == 'Illness':
            x_mark_enc = np.zeros((seq_len, 2))
            x_mark_dec = np.zeros((label_len + pred_len, 2))
        elif dataset == 'Exchange':
            x_mark_enc = np.zeros((seq_len, 3))
            x_mark_dec = np.zeros((label_len + pred_len, 3))
...
def train_timesnet_model(dataset, X_train_enc, y_train, X_test_enc, y_test,
    ...
    if dataset == 'Illness':
        x_mark_enc_train = torch.zeros(X_train_enc.shape[0], X_train_enc.shape[1], 2).to(device)
        x_mark_dec_train = torch.zeros(X_train_dec.shape[0], X_train_dec.shape[1], 2).to(device)
    elif dataset == 'Exchange':
        x_mark_enc_train = torch.zeros(X_train_enc.shape[0], X_train_enc.shape[1], 3).to(device)
        x_mark_dec_train = torch.zeros(X_train_dec.shape[0], X_train_dec.shape[1], 3).to(device)
    elif dataset == 'ETTm1' or dataset == 'Weather':
    ...
    with torch.no_grad():
        ...
        if dataset == 'Illness':
            x_mark_enc_test = torch.zeros(X_test_enc.shape[0], X_test_enc.shape[1], 2).to(device)
            x_mark_dec_test = torch.zeros(X_test_dec.shape[0], X_test_dec.shape[1], 2).to(device)
        elif dataset == 'Exchange':
            x_mark_enc_test = torch.zeros(X_test_enc.shape[0], X_test_enc.shape[1], 3).to(device)
            x_mark_dec_test = torch.zeros(X_test_dec.shape[0], X_test_dec.shape[1], 3).to(device)
        elif dataset == 'ETTm1' or dataset == 'Weather':
...
def train_evaluate_timesnet(X_train_enc, y_train, X_val_enc, y_val,
    ...
    predictions = train_timesnet_model(
        dataset, X_train_enc, y_train, X_val_enc, y_val,
        X_train_dec, X_val_dec,
        configs, device,
...
for dataset, columns in datasets.items():
    ...
    timesnet_param = dataset_param["timesnet_param"]
    ...
    x_enc, x_mark_enc, x_dec, x_mark_dec, y = create_timesnet_dataset(
        scaled_features, seq_len, label_len, pred_len, dataset
    )
    ...
    configs = Configs(
        ...
        label_len=label_len,
        pred_len=pred_len,
        e_layers=timesnet_param['e_layers'],
        dataset_type=dataset,
        top_k=timesnet_param.get('top_k', 5),
        num_kernels=timesnet_param.get('num_kernels', 6)
    )

    predictions_clean = train_timesnet_model(
        dataset, x_enc_train, y_train, x_enc_test, y_test,
        x_dec_train, x_dec_test,
        configs, device,
        num_epochs=timesnet_param['num_epochs'],
        batch_size=timesnet_param['batch_size'],
        lr=timesnet_param['lr']
    )
        ...
        for corr in corr_list:
            for ingredient in Ingredients:
                ...
                x_enc_dirty, x_mark_enc_dirty, x_dec_dirty, x_mark_dec_dirty, y_dirty = create_timesnet_dataset(
                    scaled_features_dirty, seq_len, label_len, pred_len, dataset
                )
                ...
                predictions_dirty = train_timesnet_model(
                    dataset, x_enc_train_dirty, y_train_dirty, x_enc_test_dirty, y_test_dirty,
    ...
    results_df.to_csv(os.path.join(output_dir, f"timesnet-decompose_change_good-results-{dataset}1.csv"), index=False)
