...
def create_micn_dataset(data, seq_len, label_len, pred_len, dataset):
    """创建MICN数据集"""
    x_enc_list = []
    x_mark_enc_list = []
    x_dec_list = []
...
def train_micn_model(dataset, X_train_enc, y_train, X_test_enc, y_test,
...
for dataset, columns in datasets.items():
    ...
    micn_param = dataset_param["micn_param"]
    ...
    predictions_clean = train_micn_model(
        ...
        configs, device,
        num_epochs=micn_param['num_epochs'],
        batch_size=micn_param['batch_size'],
        lr=micn_param['lr']
    )
    ...
    results_df.to_csv(os.path.join(output_dir, f"micn-decompose_change_good-results-{dataset}1.csv"), index=False)
