...
Imputation_Algorithms = ['Mean', 'Median', 'Mode', 'KNN', 'HDI',
                         'MICE',
                         'IIM', 'SI',
                         'MFI', 'MissFI', 'XGBI',
    'GAIN',
...
#Missing_rate = [ 55, 60, 65, 70, 75, 80, 85, 90, 95]
Mechanism = ["MNAR", ]
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

param_grid = {
    'd_layers': [1],  # SCINet stacks (1 or 2)
    'num_epochs': [1],
    'batch_size': [64],
    'lr': [0.001],
    'current_level': [1],  # SCINet tree depth
    'kernel_size': [2]  # SCINet kernel size
}

for dataset, columns in datasets.items():
    print('-' * 70)
    print(f"{dataset}: Processing with SCINet...")

    target_column = columns["target_column"]
    base_path = "../../Datasets"

    scinet_param = scinet_params[dataset]
    seq_len = scinet_param["seq_len"]
    label_len = scinet_param["label_len"]
    pred_len = scinet_param["pred_len"]
    ...
    for pattern in Mechanism:
        ...
        #            'current_level': random.choice(param_grid['current_level']),
        ...
        #        rmse, mae = train_evaluate_scinet(
        #            X_train_enc, y_train, X_val_enc, y_val,
        #            X_train_dec, X_val_dec,
        ...
        #              f"current_level={params['current_level']}, kernel_size={params['kernel_size']}, "
        ...
        configs = Configs(
            ...
            current_level=best_cv_params.get('current_level', 3),
            kernel_size=best_cv_params.get('kernel_size', 5)
        )
        ...
        for method in Imputation_Algorithms:
            for rate in Missing_rate:
                ...
                x_enc_imp, x_mark_enc_imp, x_dec_imp, x_mark_dec_imp, y_imp = create_scinet_dataset(
                ...
                predictions_imp = train_scinet_model(
        ...
        output_results_file = os.path.join(output_results_path, f"scinet-imputation-results-{dataset}.csv")
