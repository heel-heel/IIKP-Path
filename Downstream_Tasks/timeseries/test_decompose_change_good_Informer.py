...
def create_informer_dataset(data, seq_len, label_len, pred_len,dataset):
    """创建Informer数据集"""
    x_enc_list = []
    x_mark_enc_list = []
    x_dec_list = []
    ...
    for i in range(total_len - seq_len - pred_len + 1):
        ...
        if dataset == 'ETTm1' or dataset == 'Weather':
...
def train_informer_model(dataset, x_enc_train, x_dec_train, y_train, x_enc_test, x_dec_test, y_test,
    ...
    if dataset == "ETTm1" or dataset == "Weather":
        ...
        x_mark_dec_train = torch.zeros(x_dec_train.shape[0], dec_len, 4).to(device)
    ...
    with torch.no_grad():
        if dataset == 'ETTm1' or dataset == 'Weather':
            ...
            x_mark_dec_test = torch.zeros(x_dec_test.shape[0], dec_len, 4).to(device)
...
def train_evaluate_informer(X_train_seq, X_train_dec, y_train, X_val_seq, X_val_dec, y_val, scaler,
    ...
    predictions = train_informer_model(
        dataset_type, X_train_seq, X_train_dec, y_train, X_val_seq, X_val_dec, y_val, configs, device,
...
for dataset, columns in datasets.items():
    ...
    informer_param = dataset_param["informer_param"]
    ...
    configs = Configs(
        ...
        label_len=label_len,
        pred_len=pred_len,
        d_model=informer_param.get('d_model', 128),
        d_ff=informer_param.get('d_ff', 512),
        e_layers=informer_param['e_layers'],
        d_layers=informer_param['d_layers'],
        n_heads=informer_param.get('n_heads', 8),
        dropout=informer_param.get('dropout', 0.1),
        factor=informer_param.get('factor', 3),
        dataset_type=dataset,
        distil=informer_param.get('distil', True)
    )

    predictions_clean = train_informer_model(
        dataset, x_enc_train, x_dec_train, y_train, x_enc_test, x_dec_test, y_test, configs, device,
        num_epochs=informer_param['num_epochs'],
        batch_size=informer_param['batch_size'],
        lr=informer_param['lr']
    )
        ...
        for corr in corr_list:
            for ingredient in Ingredients:
                ...
                predictions_dirty = train_informer_model(
                    dataset, x_enc_train_dirty, x_dec_train_dirty, y_train_dirty,
    ...
    results_df.to_csv(os.path.join(output_dir, f"informer-decompose_change_good-results-{dataset}1.csv"), index=False)
