...
def train_evaluate_timemixer_early_stopping_false(X_train, y_train, X_test, y_test, scaler, best_look_back, config_params):
    set_seed(42)
...
def train_evaluate_timemixer_early_stopping_true(X_train, y_train, X_test, y_test, scaler, best_look_back, config_params):
    set_seed(42)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for train_index, test_index in tscv.split(X):
        ...
        for params in param_combinations:
            if params['early_stopping']:
                current_rmse, current_mae = train_evaluate_timemixer_early_stopping_true(X_train, y_train, X_test, y_test,
    ...
    for model in Imputation_Algorithms:
        for rate in Missing_rate:
            ...
            if best_cv_params['early_stopping']:
                ...
                rmse, mae = train_evaluate_timemixer_early_stopping_false(X_train_i, y_train_i, X_test_i, y_test, scaler,
                                                                        best_look_back, best_cv_params)
