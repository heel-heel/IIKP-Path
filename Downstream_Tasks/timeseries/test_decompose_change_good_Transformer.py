...
for dataset, columns in datasets.items():
    ...
    transformer_param = dataset_param["transformer_param"]
    ...
    configs = Configs(
        ...
        label_len=label_len,
        pred_len=pred_len,
        e_layers=transformer_param['e_layers'],
        d_layers=transformer_param['d_layers']
    )

    predictions_clean = train_transformer_model(
        x_enc_train, y_train, x_enc_test, y_test, configs, device,
        num_epochs=transformer_param['num_epochs'],
        batch_size=transformer_param['batch_size'],
        lr=transformer_param['lr']
    )
    ...
    results_df.to_csv(os.path.join(output_dir, f"transformer-decompose_change_good-results-{dataset}1.csv"), index=False)
