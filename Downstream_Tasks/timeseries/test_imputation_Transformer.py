...
def create_transformer_dataset(data, seq_len, label_len, pred_len):
    x_enc_list = []
    x_mark_enc_list = []
    x_dec_list = []
    x_mark_dec_list = []
...
def train_transformer_model(X_train, y_train, X_test, y_test, configs,
    ...
    model = TransformerModel(configs).to(device)
...
def train_evaluate_transformer(X_train_seq, y_train, X_val_seq, y_val, scaler,
    ...
    print(444)
    predictions = train_transformer_model(
...
for dataset, columns in datasets.items():
    print('-' * 70)
    print(f"{dataset}: Processing with Transformer...")

    target_column = columns["target_column"]
    base_path = "../../Datasets"

    trans_param = transformer_params[dataset]
    seq_len = trans_param["seq_len"]
    label_len = trans_param["label_len"]
    pred_len = trans_param["pred_len"]
    ...
    for pattern in Mechanism:
        ...
        for i, col in enumerate(feature_columns):
            ...
            scalers[col] = scaler
        x_enc, x_mark_enc, x_dec, x_mark_dec, y = create_transformer_dataset(
        ...
        #        rmse, mae = train_evaluate_transformer(
        ...
        predictions_clean = train_transformer_model(
        ...
        for rate in Missing_rate:
            ...
            x_enc_dirty, x_mark_enc_dirty, x_dec_dirty, x_mark_dec_dirty, y_dirty = create_transformer_dataset(
            ...
            predictions_dirty = train_transformer_model(
        ...
        for method in Imputation_Algorithms:
            for rate in Missing_rate:
                ...
                x_enc_imp, x_mark_enc_imp, x_dec_imp, x_mark_dec_imp, y_imp = create_transformer_dataset(
                ...
                predictions_imp = train_transformer_model(
        ...
        output_results_file = os.path.join(output_results_path, f"transformer-imputation-results-{dataset}.csv")
