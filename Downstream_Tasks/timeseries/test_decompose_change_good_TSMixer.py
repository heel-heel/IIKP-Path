    ...
    for portion in portion_list:
        for corr in corr_list:
            for ingredient in Ingredients:
                ...
                X_imp, y_imp = create_dataset(target_scaled_test, look_back)
                X_train_i, X_test_i = X_imp[:split_idx], X_imp[split_idx:]
                y_train_i, y_test_i = y_imp[:split_idx], y_imp[split_idx:]

                if tsmixer_param['early_stopping'] == True:
                    rmse, mae = train_evaluate_tsmixer_early_stopping_true(X_train_i, y_train_i, X_test_i, y_test, scaler, look_back, tsmixer_param)
                elif tsmixer_param['early_stopping'] == False:
                    rmse, mae = train_evaluate_tsmixer_early_stopping_false(X_train_i, y_train_i, X_test_i, y_test, scaler, look_back, tsmixer_param)
