...
global_median = data['Temp'].median()
# 随机选择10%的数据点设置为NaN
...
# 这里设置窗口大小为5，可以根据需要调整
window_size = 5
data['Temp'] = data['Temp'].fillna(data['Temp'].rolling(window=window_size, min_periods=1).median())

data['Temp'] = data['Temp'].fillna(global_median)
# 保存处理后的数据
