...
def process_and_fill(input_file, output_file, target_column, nonnumerical_column):
    ...
    global_median = data[target_column].median()
    data[target_column] = data[target_column].fillna(global_median)
...
for dataset, columns in datasets.items():
    ...
    output_path = os.path.join(base_path, dataset, "Imputation", "null-median")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    for rate in Missing_rate:
        input_file = os.path.join(input_path, f'dirty-{rate}.csv')
        output_file = os.path.join(output_path, f'dirty-median-{rate}')
