...
def process_and_fill(input_file, output_file, target_column, nonnumerical_column):
    ...
    # 记录原始数据类型
    original_dtypes = df_copy[target_column].dtype if target_column in df_copy.columns else None
    ...
    # 数据采样：如果数据量过大，进行采样
    max_samples = 5000
    if len(complete_df) > max_samples:
        print(f"完整数据量 {len(complete_df)} 超过限制 {max_samples}，进行随机采样")
        complete_df = complete_df.sample(n=max_samples, random_state=42)

    scaler = StandardScaler()
    X_complete_scaled = scaler.fit_transform(complete_df[feature_columns])
    X_incomplete_scaled = scaler.transform(incomplete_df[feature_columns]) if len(incomplete_df) > 0 else np.array([])

    y_complete = complete_df[target_column].values

    # ==================== 优化后的 learning_phase ====================
    def learning_phase(X_complete, y_complete, l_values):
        """优化内存版本的 learning_phase"""
        n_complete = len(X_complete)
        models_dict = {}

        print(f"开始学习阶段，样本数: {n_complete}")

        for i in range(n_complete):
            if i % 500 == 0:
                print(f"  处理进度: {i}/{n_complete}")

            current_sample = X_complete[i]
            current_value = y_complete[i]

            # 使用分批计算距离，避免一次性分配大数组
            distances = np.zeros(n_complete)

            # 分批计算距离
            batch_size = 1000
            for start in range(0, n_complete, batch_size):
                end = min(start + batch_size, n_complete)
                batch = X_complete[start:end]
                batch_distances = np.linalg.norm(batch - current_sample, axis=1)
    ...
    def select_optimal_models(models_dict, X_complete, y_complete, k_validation=5):
        ...
        for i in range(n_complete):
            ...
            distances[i] = np.inf
            sorted_indices = np.argsort(distances)
            validation_indices = sorted_indices[:k_validation]
    ...
    def imputation_phase(X_incomplete, X_complete, y_complete, optimal_models, k_imputation=5):
        ...
        print(f"开始插补阶段，待插补样本数: {len(X_incomplete)}")

        for idx, incomplete_sample in enumerate(X_incomplete):
            if idx % 500 == 0:
                print(f"  处理进度: {idx}/{len(X_incomplete)}")
            ...
            for start in range(0, n_complete, batch_size):
                ...
                batch = X_complete[start:end]
                batch_distances = np.linalg.norm(batch - incomplete_sample, axis=1)
                distances[start:end] = batch_distances
            ...
            for neighbor_idx in imputation_neighbors:
                model_info = optimal_models.get(neighbor_idx)
            ...
            if len(candidate_predictions) > 0:
                ...
                predictions.append(np.mean(y_complete))
    ...
    if len(X_incomplete_scaled) > 0:
        ...
        for idx, pred_value in zip(incomplete_df.index, predictions):
            if original_dtypes in ['int64', 'int32']:
                df_copy.at[idx, target_column] = int(round(pred_value))
            else:
