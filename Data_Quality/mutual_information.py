        ...
        for pattern in Mechanism:
            ...
            for model in Imputation_Algorithms:
                for rate in Missing_rate:
                    input_dirty_file = os.path.join(base_path, dataset, "Imputation", pattern, f'null-{model}', f'dirty-{model}-{rate}.csv')
                    if nonnumerical_column != "None":
                         dirty_df = pd.read_csv(input_dirty_file).drop(columns=[nonnumerical_column])
                    else:
                        dirty_df = pd.read_csv(input_dirty_file)
                    assert list(clean_df.columns) == list(dirty_df.columns), "Must be same"
            ...
            results_df.to_csv(os.path.join(output_path, f'mutual_information_results_{pattern}.csv'), index=False)
            ...
            ax.set_title(f"Mutual Information between Clean and Dirty Data for {dataset} under {pattern}", fontsize=16)
            ...
            plt.savefig(os.path.join(output_fig_path, f'mutual_information_plot_{pattern}.png'))
            plt.show()
