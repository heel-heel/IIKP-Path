...
for dataset, columns in datasets.items():
    ...
    for input_dirty_file in input_dirty_files:
        if '50' in input_dirty_file:
            ...
            plt.plot(sorted_dataset[sorted_dataset >= 0], yvals[sorted_dataset >= 0], label=model_name, linewidth=1.5)

    plt.legend(loc='lower right', fontsize='small')
    plt.title(f'Cumulative Distribution Functions (CDFs) for {dataset}')
    plt.xlabel('Value')
