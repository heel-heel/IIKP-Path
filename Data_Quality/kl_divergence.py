...
def evaluate_data_quality(datasets, Imputation_Algorithms, Missing_rate, Mechanism):
    ...
    for dataset, columns in datasets.items():
        ...
        for pattern in Mechanism:
            ...
            results_df.to_csv(os.path.join(output_path, f"kl_divergence_results_{pattern}.csv"), index=False)
            print(f"KL Divergence results saved to {os.path.join(output_path, f'kl_divergence_results_{pattern}.csv')}")

            # bar chat
            #colors = plt.cm.viridis(np.linspace(0, 1, len(Imputation_Algorithms)))  # different color for different models
            ...
            for rate in Missing_rate:
                ...
                plt.title(f'KL Divergence for {rate}% Missing Data in {dataset} under {pattern}',fontsize=16)
                ...
                output_fig_path = os.path.join(output_path, f"fig", pattern)
